#!/usr/bin/env bash
#
# Kareya Silo — one-command installer.
#
...
# Silo stays entirely under your control, and Kareya never touches it.
#
...
#   --repo   <o/r>    GitHub repo to install from (default: sengtha/Kareya-Silo).
#   --branch <name>   Branch to install (default: main).
#   --dir    <path>   Install directory (default: /opt/kareya-silo).
...
# into .env automatically: OPENAI_API_KEY, HUB_URL, HUB_ANON_KEY,
# HUB_PUBLISHABLE_KEY, HUB_SILO_API_KEY.
#
# After the stack is up, register the Silo on the Kareya Hub: create a
# `user_silos` row with this Silo's URL + its ANON_KEY (docker/.env). See
# docs/Deploy-from-GitHub.md.

set -euo pipefail

REPO="sengtha/Kareya-Silo"
BRANCH="main"
DIR="/opt/kareya-silo"
DOMAIN=""
EMAIL=""
...
if [ -n "${OPENAI_API_KEY:-}${HUB_URL:-}${HUB_ANON_KEY:-}${HUB_PUBLISHABLE_KEY:-}${HUB_SILO_API_KEY:-}" ]; then
  log "Applying optional feature keys from environment…"
  setenv OPENAI_API_KEY      "${OPENAI_API_KEY:-}"
  setenv HUB_URL             "${HUB_URL:-}"
  setenv HUB_ANON_KEY        "${HUB_ANON_KEY:-}"
...
ANON_KEY_VALUE="$(grep -E '^ANON_KEY=' .env | head -1 | cut -d= -f2- || true)"

echo
printf '\033[1;32m✓ Kareya Silo is starting.\033[0m\n'
...
echo "  3. Register the Silo on the Kareya Hub — create a \`user_silos\` row with:"
echo "       Project URL:  https://${DOMAIN}"
echo "       Anon Key:     ${ANON_KEY_VALUE:-see ${DIR}/docker/.env (ANON_KEY)}"
echo "     Your \`employees\` row is auto-provisioned from your Hub role on first login."
echo "  4. For Web Push, add the Silo's HUB_SILO_API_KEY (from the Hub's Silo"
echo "     registry) to ${DIR}/docker/.env and re-run \`docker compose up -d\`."
